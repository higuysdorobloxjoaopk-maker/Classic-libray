--// MODERNIZED GUI VERSION (No UICorner)
--// iOS Style Toggle (Custom)
--// Fully Functional Close Button

local library = {}
library.Name = "$classicle libray(Classic-lib base)"
local windowCount = 0
local sizes = {}
local listOffset = {}
local windows = {}
...
if game.CoreGui:FindFirstChild('ClassicUiLib') then
    game.CoreGui:FindFirstChild('ClassicUiLib'):Destroy()
    destroyed = true
end
...
local ClassicUiLib = Instance.new("ScreenGui")

ClassicUiLib.Name = "ClassicUiLib"

protect_gui(ClassicUiLib)
...
function library:Destroy()
    ClassicUiLib:Destroy()
    if keybindConnection then
        keybindConnection:Disconnect()
    end
end
function library:Hide()
   ClassicUiLib.Enabled = not ClassicUiLib.Enabled
end	
...
function library:SetAllToggles(state)
    for _,v in pairs(globalToggles) do
        v:Set(state)
    end
end
...
function library:Window(name) 
    ...
    UiWindow.Parent = ClassicUiLib
    UiWindow.BackgroundColor3 = Color3.fromRGB(255,255,255)
    UiWindow.BackgroundTransparency = 0.75
    ...
    CloseButton.Position = UDim2.new(0,160,0,2)
    CloseButton.Text = "X"
    ...
    function functions:Toggle(text, on, callback)
        ...
        ToggleButton.MouseButton1Click:Connect(function()
            local newState = not ToggleFiller.Visible
            ToggleFiller.Visible = newState
            ToggleFiller:TweenSize(
                newState and UDim2.new(0,16,0,16) or UDim2.new(0,12,0,12),
                Enum.EasingDirection.Out,
                Enum.EasingStyle.Quad,
                0.15,
                true
            )
            ToggleButton.BackgroundColor3 = newState and Color3.fromRGB(0,170,255) or Color3.fromRGB(200,200,200)
            callback(newState)
        end)
        ...
        function toggleObject:Set(val)
            ToggleFiller.Visible = val
            callback(val)
        end
        table.insert(globalToggles, toggleObject)
...
--==============================--
-- VISUAL IMPROVEMENTS INJECTED
--==============================--

local TweenService = game:GetService("TweenService")

-- Remove UICorner objects
...
for _, obj in pairs(game:GetDescendants()) do
    if obj:IsA("Frame") then
        obj.BackgroundColor3 = Color3.fromRGB(25,25,28)
        obj.BorderSizePixel = 0
    end
end

-- Improve all TextButtons
for _, obj in pairs(game:GetDescendants()) do
    if obj:IsA("TextButton") then
        obj.BackgroundColor3 = Color3.fromRGB(35,35,40)
        ...
        obj.MouseEnter:Connect(function()
            TweenService:Create(obj, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(50,50,55)}):Play()
        end)
        
        obj.MouseLeave:Connect(function()
            TweenService:Create(obj, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(35,35,40)}):Play()
        end)
    end
end

--==============================--
-- IOS STYLE TOGGLE (No UICorner)
--==============================--
...
for _, obj in pairs(game:GetDescendants()) do
    if obj:IsA("TextButton") and string.lower(obj.Name):find("close") then
